...
def getNewLogger(name:str) -> logging.Logger:
	...
	logLevel = os.environ.get('backend_log_level')
	if logLevel == None:
		logLevel = 'INFO'
	log.setLevel(logging._nameToLevel[logLevel.upper()])
	stdout_handler = logging.StreamHandler()
	stdout_handler.setLevel(logging._nameToLevel[os.environ.get('backend_log_level').upper()])
	# https://docs.python.org/3/library/logging.html#logrecord-attributes
	stdout_handler.setFormatter(CustomLogFormat('%(asctime)s - ^COL_START^%(levelname)s^COL_END^:     %(name)s:	%(message)s'))
	log.addHandler(stdout_handler)
	return log

logger = getNewLogger('backend')